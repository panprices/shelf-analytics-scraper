...
import { MacysCrawlerDefinition } from "./custom/macys.js";
  ...
  static async buildPlaywrightCrawler(
    ...
    const antiBotDetectionOptions: PlaywrightCrawlerOptions = {
      ...
      proxyConfiguration: new ProxyConfiguration({
        newUrlFunction: async (_sessionId) => {
          if (launchOptions.ip) {
            return `http://${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@${launchOptions.ip}:60000`;
          }
          const availableIp = await newAvailableIp(firestore, domain);
          return `http://${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@${availableIp}:60000`;
        },
      }),
    };
    ...
    switch (domain) {
      ...
      case "royaldesign.se":
        ...
        return [new PlaywrightCrawler(options), definition];
      case "macys.com":
        definition = await MacysCrawlerDefinition.create(
          launchOptions
        );
        options = {
          ...defaultOptions,
...
export const proxyConfiguration = {
  ...
  DE: new ProxyConfiguration({
    proxyUrls: ["http://${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@panprices.oxylabs.io:60003"],
  }),
  SE: new ProxyConfiguration({
    proxyUrls: ["http://${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@panprices.oxylabs.io:60002"],
  }),
  ...
  SE_CRAWLEE_IP_ROTATE: new ProxyConfiguration({
    proxyUrls: [
      ...
      "103.69.158.205",
      "103.69.158.207",
    ].map((ip) => `http://${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@${ip}:60000`),
  }),
  ...
  //   proxyUrls: ["http://sdc${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@dc.pr.oxylabs.io:10000"],
  // }),
  // SHARED_DATACENTER_DE: new ProxyConfiguration({
  //   proxyUrls: ["http://sdc${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@dc.de-pr.oxylabs.io:40000"],
  // }),
  // SHARED_DATACENTER_FR: new ProxyConfiguration({
  //   proxyUrls: ["http://sdc${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@dc.fr-pr.oxylabs.io:42000"],
  // }),
  // SHARED_DATACENTER_UK: new ProxyConfiguration({
  //   proxyUrls: ["http://sdc${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@dc.gb-pr.oxylabs.io:46000"],
  // }),
  // SHARED_DATACENTER_NL: new ProxyConfiguration({
  //   proxyUrls: ["http://sdc${process.env.PROXY_USERNAME}:${process.env.PROXY_PASSWORD}@dc.nl-pr.oxylabs.io:44000"],
  // }),
};
