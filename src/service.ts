        ...
        return scrapeDetails(requestOptions, overrides).then(
          (detailedProducts) => {
            ...
            if (detailedProducts.length < listingProducts.length) {
              log.warning("Some products failed to scrape", {
                expected: listingProducts.length,
                actual: detailedProducts.length,
                missing: listingProducts.length - detailedProducts.length
              });
            }
...
function postProcessProductDetail(
  ...
  if (p.gtin) {
    if (!isValidGTIN(p.gtin)) {
      ...
      // Don't pad any GTIN - keep them in their original format
      // GTIN-8: 8 digits
